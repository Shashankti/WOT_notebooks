    ...
    **kwargs : dict
    ...
    def __init__(self, matrix, day_field='day', covariate_field='covariate',
        ...
        if solver == 'fixed_iters':
            self.solver = transport_stablev2
    ...
    def compute_single_transport_map(self, config):
        ...
        if self.lambda1_array in p0.obs.columns:
            config['lambda1'] = np.array(p0.obs[self.lambda1_array].tolist())
        else:
            config['lambda1'] = config['lambda1']
        
        if self.lambda2_array in p0.obs.columns:
            config['lambda2'] = np.array(p1.obs[self.lambda2_array].tolist())
        else:
        ...
        if self.growth_average in p1.obs.columns:
            ...
            config['G_avg'] = np.ones(len(p1.obs))
