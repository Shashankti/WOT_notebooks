...
def primal(C, K, R, dx, dy, p, q, a, b, epsilon, lambda1, lambda2):
    ...
    with np.errstate(divide='ignore'):
        return F1(np.dot(R, dy), dx) + F2(np.dot(R.T, dx), dy) \
               + (np.sum(epsilon * R * np.nan_to_num(np.log(R)) - R + K) \
                  + np.sum(R * C)) / (I * J)
...
def dual(C, K, R, dx, dy, p, q, a, b,epsilon_ij, epsilon_i,epsilon_j, lambda1, lambda2):
    I = len(p)
    J = len(q)
    F1c = lambda u, v: fdivstar(lambda1, u, p, v)
    F2c = lambda u, v: fdivstar(lambda2, u, q, v)
    return - F1c(- epsilon_i * np.log(a), dx) - F2c(- epsilon_j * np.log(b), dy) \
           - np.sum(epsilon_ij*(R - K)) / (I * J)
...
def optimal_transport_duality_gap(C, G, G_avg, lambda1, lambda2, epsilon, batch_size, tolerance, tau,
                                  epsilon0, max_iter,epsilon_row,epsilon_col, **ignored):
    """
    ...
    epsilon_scalings = 5
    scale_factor_i = np.exp(- np.log(epsilon_row) / epsilon_scalings)
    scale_factor_j = np.exp(- np.log(epsilon_col) / epsilon_scalings)
    scale_factor_ij = np.exp(- np.log(epsilon) / epsilon_scalings)
...
#    q = np.ones(C.shape[1])* (C.shape[1]/C.shape[0]) 
    ...
    epsilon_i = epsilon0 * scale_factor_i
    epsilon_j = epsilon0 * scale_factor_j
    epsilon_ij = epsilon0 * scale_factor_ij
    ...
    for e in range(epsilon_scalings + 1):
        ...
        v = v + epsilon_j * np.log(b)  # absorb
        # print('this is the starting v',v)
        epsilon_i = epsilon_i / scale_factor_i
        epsilon_j = epsilon_j / scale_factor_j
        epsilon_ij = epsilon_ij / scale_factor_ij
        _K = np.exp(-C / epsilon_ij)
        #_K = np.exp((-C * 
        alpha1 = lambda1 / (lambda1 + epsilon_i)
        alpha2 = lambda2 / (lambda2 + epsilon_j)
        K = np.exp((np.array([u]).T - C + np.array([v])) / epsilon_ij)
        a, b = np.ones(I), np.ones(J)
        old_a, old_b = a, b
        ...
        while duality_gap > threshold:
            for i in range(batch_size if e == epsilon_scalings else 5):
                ...
                b = (q / (K.T.dot(np.multiply(a, dx)))) ** alpha2 * np.exp(-v / (lambda2 + epsilon_j))

                # stabilization
                if (max(max(abs(a)), max(abs(b))) > tau):
                    print("We are now stabalizing")
                    u = u + epsilon_i * np.log(a)
                    v = v + epsilon_j * np.log(b)  # absorb
                    K = np.exp((np.array([u]).T - C + np.array([v])) / epsilon_ij)
                    a, b = np.ones(I), np.ones(J)
            ...
            if e == epsilon_scalings:
                R = (K.T * a).T * b
                pri = primal(C, _K, R, dx, dy, p, q, _a, _b, epsilon_ij, lambda1, lambda2)
                dua = dual(C, _K, R, dx, dy, p, q, _a, _b, epsilon_ij,epsilon_i,epsilon_j, lambda1, lambda2)
                ...
                duality_gap = max(
                    ...
                    np.linalg.norm(_b - old_b * np.exp(v / epsilon_j)) / (1 + np.linalg.norm(_b)))
